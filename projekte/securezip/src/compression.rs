...
pub fn compress(data: &[u8]) -> Result<Vec<u8>, SecureZipError> {
    ...
    encoder.write_all(data).map_err(|e| SecureZipError::Compression(e.to_string()))?;
    let compressed = encoder.finish().map_err(|e| SecureZipError::Compression(e.to_string()))?;
    Ok(compressed)
}
...
pub fn decompress(data: &[u8]) -> Result<Vec<u8>, SecureZipError> {
    ...
    decoder.read_to_end(&mut decompressed).map_err(|e| SecureZipError::Compression(e.to_string()))?;
    Ok(decompressed)
}
